...
static int detect_collision(struct fb_copyarea *area_a,
			    struct fb_copyarea *area_b)
{
	int a_x0 = area_a->dx;
...
{
	struct fb_copyarea *snake_head = get_snake_head();
	for (size_t j = tail_index(); j != head_index();
	     j = ((j + 1) % SNAKE_MAX_LENGTH)) {
...
static inline void button_action(int button_state)
{
	...
	int newly_pressed =
	    (prev_button_state ^ button_state) & prev_button_state;
...
int main()
{
	...
	game_screen =
	    mmap(NULL, SCREEN_WIDTH * SCREEN_HEIGHT * 2, PROT_WRITE, MAP_SHARED,
		 fb_fd, 0);
